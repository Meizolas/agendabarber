...
import type { AppointmentPaymentMethod, AvailabilityRule, Barber, Service, StaffMember } from '@/types'
...
type PublicPlan = { plan_code?: string; staff_limit?: number }
...
const demoBarber: Barber = {
  ...
  whatsapp: '11999999999', slug: 'demo', logo_url: null, pix_key: '11999999999', pix_key_type: 'phone', created_at: '', updated_at: '',
}
...
export default function BookingPage() {
  ...
  const [plan, setPlan] = useState<PublicPlan>({ staff_limit: 1 })
  ...
  useEffect(() => {
    const load = async () => {
      if (slug === 'demo') {
        ...
        setStaff(demoStaff)
        setPlan({ plan_code: 'team', staff_limit: 3 })
        setRules(demoRules)
        setLoading(false)
        return
      }
      ...
      setPlan(payload.plan ?? { staff_limit: 1 })
  ...
  const handleSubmit = async (clientData: ClientFormValues) => {
    ...
    if (barber.id === 'demo') {
      ...
      setBooking((current) => ({ ...current, clientName: clientData.client_name, appointmentId: 'DEMO123' }))
      setStep('success')
      setSubmitting(false)
      return
    }
    ...
    const response = await fetch('/api/appointments', {
      ...
      body: JSON.stringify({
        ...
        appointment_time: booking.time, notes: clientData.notes, payment_method: booking.paymentMethod,
      }),
    })
    const payload = await response.json().catch(() => null)
    if (response.ok) {
  ...
  const staffSelectionEnabled = (plan.staff_limit ?? staff.length) > 1 && staff.length > 1
  const orderedSteps: Exclude<Step, 'success'>[] = staffSelectionEnabled ? ['staff', 'service', 'date', 'time', 'client'] : ['service', 'date', 'time', 'client']
  const firstStaffMember = staff[0] ?? null
  const stepLabels: Record<Exclude<Step, 'success'>, { eyebrow: string; title: string }> = {
    staff: { eyebrow: 'Profissional', title: 'Escolha o barbeiro' },
    service: { eyebrow: 'Serviço', title: 'Escolha o serviço' },
    date: { eyebrow: 'Data', title: 'Escolha a data' },
    time: { eyebrow: 'Horário', title: 'Escolha o horário' },
    client: { eyebrow: 'Confirmar', title: 'Confirme seu agendamento' },
  }
  const returnStep = () => {
  ...
  const currentIndex = step === 'success' ? orderedSteps.length - 1 : Math.max(0, orderedSteps.indexOf(step))
  const currentStepKey = orderedSteps[currentIndex]
  const pixPayload = barber?.pix_key && booking.paymentMethod === 'pix' && booking.service
    ? buildPixPayload({
      pixKey: barber.pix_key,
      pixKeyType: barber.pix_key_type ?? 'phone',
      merchantName: barber.barbershop_name,
      amount: booking.service.price,
      txid: booking.appointmentId || `${booking.date ?? ''}${booking.time ?? ''}`,
      description: `Agendamento ${booking.service.name}`,
    })
    : ''

  useEffect(() => {
    if (loading || staffSelectionEnabled || !firstStaffMember) return
    setBooking((current) => current.staffMember ? current : { ...current, staffMember: firstStaffMember })
    if (step === 'staff') setStep('service')
  }, [firstStaffMember, loading, staffSelectionEnabled, step])
      ...
      <div className="mx-auto min-h-screen w-full max-w-[430px] border-x border-white/[0.06] bg-[#080A0C] pb-5 shadow-[0_0_80px_rgba(0,0,0,0.7)]">
        {step === 'success' && booking.staffMember && booking.service && booking.date && booking.time && barber ? (
          ...
          <>
            ...
            <div className="px-4">
              <StepProgress current={currentIndex} total={orderedSteps.length} />
              <motion.section key={step} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.22 }} className="pt-4">
                ...
                <p className="text-[10px] font-semibold text-[#F5C400]">{currentIndex + 1} • {stepLabels[currentStepKey].eyebrow}</p>
                <h1 className="mb-3 mt-1 text-[22px] font-semibold leading-tight">{stepLabels[currentStepKey].title}</h1>

                {step === 'staff' && staffSelectionEnabled && <StaffSelector staff={staff} selectedId={booking.staffMember?.id ?? null} onSelect={selectStaff} />}
                ...
                {step === 'client' && booking.staffMember && booking.service && booking.date && booking.time && <><ConfirmationSummary staffMember={booking.staffMember} service={booking.service} date={booking.date} time={booking.time} /><ClientForm onSubmit={handleSubmit} loading={submitting} paymentMethod={booking.paymentMethod} onPaymentMethodChange={(paymentMethod) => setBooking((current) => ({ ...current, paymentMethod }))} pixAvailable={Boolean(barber.pix_key)} /></>}
...
function StepProgress({ current, total }: { current: number; total: number }) {
  ...
  return <div className="relative mt-1 flex items-center justify-between"><span className="absolute left-4 right-4 top-1/2 h-px -translate-y-1/2 bg-white/20" /><span className="absolute left-4 top-1/2 h-px -translate-y-1/2 bg-[#F5C400] transition-all" style={{ width: `calc((100% - 2rem) * ${progress})` }} />{steps.map((number, index) => <span key={number} className={`relative z-10 grid h-6 w-6 place-items-center rounded-full border text-[10px] ${index <= current ? 'border-[#F5C400] bg-[#F5C400] text-black' : 'border-white/25 bg-[#17191C] text-[#A2A6AD]'}`}>{number}</span>)}</div>
}
